...
class Tsch(object):
    ...
    def addCell(self, slotOffset, channelOffset, neighbor, cellOptions, slotframe_handle=0, trackId=None):
        ...
        # add cell
        cell = Cell(slotOffset, channelOffset, cellOptions, neighbor, False, trackId)
        if cell.trackId is not None:
            self.mote.trae.register_track_cell(cell)
        slotframe.add(cell)
    ...
    def deleteCell(self, slotOffset, channelOffset, neighbor, cellOptions, slotframe_handle=0):
        ...
        # unregister from Track Resource Adaption Engine
        if cell.trackId is not None:
            self.mote.trae.delete_track_cell(cell)
    ...
    def get_first_packet_to_send(self, dst_mac_addr=None, trackId=None):
        ...
        if dst_mac_addr is None:
            ...
            if trackId is not None:
                for _type in [d.PKT_TYPE_DATA_TRACKED_CRITICAL, d.PKT_TYPE_DATA]:
                    for packet in self.txQueue:
                ...
                for packet in self.txQueue:
                    ...
                    if packet['mac']['dstMac'] == dst_mac_addr:
                        if packet['type'] == d.PKT_TYPE_DATA_TRACKED_CRITICAL:
                            continue
                        return packet
    ...
    def txDone(self, isACKed, channel):
        ...
        if self.pktToSend['mac']['dstMac'] == d.BROADCAST_ADDRESS:
            ...
            if isACKed:
                ...
                if (
                        ...
                        and
                        (self.pktToSend['type'] not in d.TRACKED_TYPES)
                    ):
        ...
        if active_cell != None and active_cell.trackId != None:
            if isACKed:
                self.mote.trae.indicate_acked_tx(active_cell, self.pktToSend['mac']['pending_bit'])
            else:
                self.mote.trae.indicate_non_acked_tx(active_cell)
    ...
    def rxDone(self, packet, channel):
        ...
        packet = new_packet
        if packet and 'mac' in packet:
            is_pending_bit_on = packet['mac']['pending_bit'] # use later
        ...
        if packet == None:
            # increase idle listenin counter for cells in track 
            if active_cell != None and active_cell.trackId != None:
                self.mote.trae.indicate_idle_rx_cell(active_cell)
                active_cell.increment_idle_rx_counter()
        ...
        if   self.mote.is_my_mac_addr(packet['mac']['dstMac']):
            ...
            if (
                    ...
                    self._is_next_slot_unused()
                    and
                    (pktType not in d.TRACKED_TYPES)
                ):
        ...
        if active_cell != None and active_cell.trackId != None:
            self.mote.trae.indicate_active_rx_cell(active_cell, is_pending_bit_on)
    ...
    def _select_active_cell(self, candidate_cells):
        ...
        for cell in candidate_cells:
            if cell.is_tx_on() and cell.get_send():
                if (
                    ...
                    _packet_to_send = self.get_first_packet_to_send(cell.mac_addr, cell.trackId)
    ...
    def _action_active_cell(self):
        ...
        if self.active_cell:
            if self.pktToSend is None:
                if self.active_cell.get_listen():
        ...
        for cell in candidate_cells:
            if cell.is_tx_on():
                if cell.trackId is not None:
                    self.mote.trae.indicate_tx_cell_elapsed(cell)
                if cell.mac_addr is not None and cell.trackId is None:
                ...
                if self.active_cell.trackId is not None:
                    cell.increment_elapsed_counter()
                    self.mote.trae.indicate_rx_cell_elapsed(cell)
                    if cell.get_listen() == False:
                        cell.increment_disabled_counter()
    ...
    def _action_TX(self, pktToSend, channel):
        ...
        if (
            ...
            if self.pending_bit_enabled and self._is_next_slot_unused():
                pending_bit = True
            else:
                pending_bit = False

        if self.active_cell is not None and self.active_cell.trackId is not None:
            condition_a = (len(
                        [
                            packet for packet in self.txQueue
                            if (
                                packet['type'] == d.PKT_TYPE_DATA_TRACKED_CRITICAL
                                and
                                (
                                    packet['app']['track'] ==
                                    self.active_cell.trackId
                                )
                            )
                        ]
                    ) > 1)
            ...
            condition_b = (
                        ...
                        and
                        self.mote.trae.doSetPendingBit(self.active_cell)
                    )
        ...
        pktToSend['mac']['pending_bit'] = pending_bit        
    ...
    def _action_tx_for_pending_bit(self):
        ...
        for packet in self.txQueue:
            if (
                    ...
                    and
                    'type' in packet and packet['type'] not in d.TRACKED_TYPES
                ):
        ...
        if self.pktToSend is None:
            ...
            # operation
            if 'type' in self.pktToSend and self.pktToSend['type'] in d.TRACKED_TYPES:
                print "PENDING_BIT_PENDING_BIT_PENDING_BIT_PENDING_BIT_PENDING_BIT_PENDING_BIT_PENDING_BIT_PENDING_BIT_PENDING_BIT_PENDING_BIT_PENDING_BIT"
            self._action_TX(
                pktToSend = self.pktToSend,
...
class Cell(object):
    ...
    def __init__(
        ...
        # mathias
        # when idle counter reaches 0, disable RX cell (why??)
        self.idle_rx_counter      = 0
        self.elapsed_counter    = 0
        self.disabled_counter   = 0
        self.trackId            = trackId
        self.listen_enable      = True
    ...
    def increment_disabled_counter(self):
        self.disabled_counter += 1

    def increment_elapsed_counter(self):
        self.elapsed_counter += 1

    def increment_idle_rx_counter(self):
        self.idle_rx_counter += 1
    ...
    def get_listen(self):
        return self.listen_enable

    def set_listen(self, listen):
        self.listen_enable = listen

    def get_send(self):
        return self.send_enable

    def set_send(self, send):
        self.send_enable = send
