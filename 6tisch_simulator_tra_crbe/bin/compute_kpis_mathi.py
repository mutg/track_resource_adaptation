...
def kpis_all(inputfile):
    ...
    for line in inputfile:
        ...
        if   logline['_type'] == SimLog.LOG_TSCH_SYNCED['type']:
            ...
            config['bePeriod'] = logline['beTxPeriod']
            ...
            if logline['packet']['type'] not in d.TRACKED_TYPES:
                continue
            mote_id    = logline['_mote_id']
            ...
            if packet['type'] in d.TRACKED_TYPES:
                ...
                if packet['type'] == d.PKT_TYPE_DATA:
                    allstats[run_id][4]['upstream_num_lost'] += 1
                if packet['type'] == d.PKT_TYPE_DATA_TRACKED_CRITICAL:
                    allstats[run_id][3]['upstream_num_lost'] += 1
...
def main():
    ...
    '''
    for infile in glob.glob(os.path.join('simData/*/', '*.dat')):
