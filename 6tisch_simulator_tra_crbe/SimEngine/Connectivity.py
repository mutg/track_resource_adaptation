    ...
    def _init_connectivity_matrix(self):
        for source in self.engine.motes:
            for destination in self.engine.motes:
                for channel in range(self.settings.phy_numChans):
                    self.connectivity_matrix[source.id][destination.id][channel] = copy.copy(
                        copy.copy(self.CONNECTIVITY_MATRIX_PERFECT_LINK)
                    )
...
class ConnectivityMaster(ConnectivityBase):
    ...
    """I want to be able to form a topology where one node has a 3 hop path to sink for which we will form a a track
       Please let me do this mr sim
        ...
        for source in self.engine.motes:
            for destination in self.engine.motes:
                for channel in range(self.settings.phy_numChans):
                    ...
                    if   source.id == 0 and destination.id == 1 or source.id == 1 and destination.id == 0:
                        connectivity = copy.copy(self.CONNECTIVITY_MATRIX_PERFECT_LINK)
                    elif source.id == 1 and destination.id == 2 or source.id == 2 and destination.id == 1:
                        connectivity = copy.copy(copy.copy(self.CONNECTIVITY_MATRIX_PERFECT_LINK))
                    elif source.id == 2 and destination.id == 3 or source.id == 3 and destination.id == 2:
                        connectivity = copy.copy(self.CONNECTIVITY_MATRIX_PERFECT_LINK)
                    elif source.id == 1 and destination.id == 4 or source.id == 4 and destination.id == 1:
                        connectivity = copy.copy(self.CONNECTIVITY_MATRIX_PERFECT_LINK)
                    elif source.id == 1 and destination.id == 5 or source.id == 5 and destination.id == 1:
                        connectivity = copy.copy(self.CONNECTIVITY_MATRIX_PERFECT_LINK)
                    elif source.id == 2 and destination.id == 7 or source.id == 7 and destination.id == 2:
                        connectivity = copy.copy(self.CONNECTIVITY_MATRIX_PERFECT_LINK)
                    elif source.id == 2 and destination.id == 6 or source.id == 6 and destination.id == 2:
                        connectivity = copy.copy(self.CONNECTIVITY_MATRIX_PERFECT_LINK)
...
class ConnectivityMSimple(ConnectivityBase):
    """I want to be able to form a topology where one node has a 3 hop path to sink for which we will form a a track
       it will look like this:
                ...
                / \\
               /   \\
             [3]   [4] <- track goes from here to 0

    """
    def _init_connectivity_matrix(self):
        link = {
            "pdr": SimEngine.SimSettings.SimSettings().pdr,
            "rssi": -10
        }
        for source in self.engine.motes:
            for destination in self.engine.motes:
                for channel in range(self.settings.phy_numChans):
                    ...
                    if   source.id == 0 and destination.id == 1 or source.id == 1 and destination.id == 0:
                        connectivity = copy.copy(link)
                    elif source.id == 1 and destination.id == 2 or source.id == 2 and destination.id == 1:
                        connectivity = copy.copy(link)
                    elif source.id == 2 and destination.id == 3 or source.id == 3 and destination.id == 2:
                        connectivity = copy.copy(link)
                    elif source.id == 4 and destination.id == 2 or source.id == 2 and destination.id == 4:
                        connectivity = copy.copy(link)
...
class ConnectivitySpecified(ConnectivityBase):
    """Random (topology) connectivity using the Pister-Hack model
    ...
    def __init__(self):
        ...
        # initialize the singleton
        super(ConnectivitySpecified, self).__init__()
    ...
    def _init_connectivity_matrix(self):
        ...
        for target_mote in self.engine.motes:
            ...
            while mote_is_deployed is False:
                ...
                if target_mote.id == 0:
                    self.coordinates[target_mote.id] = (0, 0)
                elif target_mote.id == 1:
                    self.coordinates[target_mote.id] = (0, 0.20)
                elif target_mote.id == 2:
                    self.coordinates[target_mote.id] = (0, 0.40)
                elif target_mote.id == 3:
                    self.coordinates[target_mote.id] = (0, 0.60)
                elif target_mote.id == 4:
                    self.coordinates[target_mote.id] = (-0.05, 0.40)
                ...
                for deployed_mote_id in self.coordinates.keys():
                    ...
                    if deployed_mote_id == target_mote.id:
                        continue
                    ...
                    pdr = self.pister_hack.convert_rssi_to_pdr(rssi)
                    # memorize the rssi and pd  r values at channel 0

                    for c in range(0, 15):
                        self._set_rssi(target_mote.id, deployed_mote_id, c, rssi)
                        self._set_pdr(target_mote.id, deployed_mote_id, c, pdr)
