...
class SimEngine(DiscreteEventEngine):
    ...
    def _init_additional_local_variables(self):
        ...
        for mote in self.motes:
            ...
            if hasattr(self.connectivity, 'coordinates'):
                motes[mote.id]['coordinates'] = self.connectivity.coordinates[mote.id]
    ...
    def _routine_thread_ended(self):
        ...
        # get idle rx / active rx for all motes
        for _mote in self.motes:
            rx_bundles = _mote.trae.track_rx_bundles
            for trackId in rx_bundles:
                self.log(
                    SimLog.LOG_TRACK_IDLE_LISTEN_RX_TOTAL,
                    {
                        '_mote_id': _mote.id,
                        'trackId': trackId,
                        'num_idle_listen_rx': sum([cell.idle_rx_counter for cell in rx_bundles[trackId]])
                    }
                )
                self.log(
                    SimLog.LOG_TRACK_RECEIVE_RX_TOTAL,
                    {
                        '_mote_id': _mote.id,
                        'trackId': trackId,
                        'num_receive_rx': sum([cell.num_rx for cell in rx_bundles[trackId]])
                    }
                )
                self.log(
                    SimLog.LOG_TRACK_DISABLED_RX_TOTAL,
                    {
                        '_mote_id': _mote.id,
                        'trackId': trackId,
                        'num_disabled_rx': sum([cell.disabled_counter for cell in rx_bundles[trackId]])
                    }
                )
                self.log(
                    SimLog.LOG_TRACK_ELAPSED_RX_TOTAL,
                    {
                        '_mote_id': _mote.id,
                        'trackId': trackId,
                        'num_elapsed_rx': sum([cell.elapsed_counter for cell in rx_bundles[trackId]])
                    }
                )
